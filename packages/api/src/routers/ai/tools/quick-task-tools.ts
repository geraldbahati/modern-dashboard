...
export const createQuickTaskTools = (userId: string) => {
  ...
  const serializeQuickTask = (task: any) => ({
    ...task,
    createdAt:
  ...
  return {
        ...
        try {
          ...
          if (params.completed !== undefined) {
            filteredTasks = tasks.filter(
              (t) => t.completed === params.completed
            );
          }
          ...
          return {
            success: true,
            data: paginatedTasks.map(serializeQuickTask),
    ...
    getQuickTaskById: tool({
      ...
      execute: async (params) => {
        try {
          const task = await QuickTaskService.getById(
            userId,
            params.quickTaskId
          );
          if (!task) {
    ...
    toggleQuickTask: tool({
      ...
      execute: async (params) => {
        try {
          const task = await QuickTaskService.toggle(
            userId,
            params.quickTaskId
          );
          return {
    ...
    completeQuickTask: tool({
      ...
      execute: async (params) => {
        try {
          const task = await QuickTaskService.update(
            userId,
            params.quickTaskId,
            {
              completed: true,
    ...
    batchCreateQuickTasks: tool({
      ...
      execute: async (params) => {
        try {
          const result = await QuickTaskService.batchCreate(
            userId,
            params.tasks
          );
          return {
            success: result.success,
            created: result.created.map(serializeQuickTask),
    ...
    batchUpdateQuickTasks: tool({
      ...
      execute: async (params) => {
        try {
          ...
          return {
            success: result.success,
            updated: result.updated.map(serializeQuickTask),
            failed: result.failed,
